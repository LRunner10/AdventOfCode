...
for y, row in enumerate(data):
    for x, char in enumerate(row):
        if char == '#': 
            ...
            characters[(x, y)] = Character(char=='G', x, y)
    ...
    for y in range(lenY):
        for x in range(lenX):
            if characters.get((x, y)) is not None:
                charcacters = characters[(x, y)].take_action(turn_count, characters)
                if all(not c.is_goblin for c in characters.values()) or all(c.is_goblin for c in characters.values()):
                    # print_board()
